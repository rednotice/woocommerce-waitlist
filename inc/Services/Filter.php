...
class Filter
{
    ...
    public function registerProductFilter() {
        global $typenow;
            ...
            <select name="slug" id="slug">
                <option value="all" <?php selected( 'all', $currentOption ); ?>><?php echo 'All Products'; ?></option>
                <?php foreach( $options as $option ) { ?>
                    <option value="<?php echo esc_attr( $option['id'] ); ?>" <?php selected( $option['id'], $currentOption ); ?>><?php echo '#' . esc_attr( $option['id'] ) . ' ' . esc_attr( $option['name'] ); ?></option>
                <?php } ?>
            </select>
        <?php }
    }
    ...
    public function filterProducts( $query ) {
        ...
        if ( is_admin() && $pagenow == 'edit.php' && $post_type == 'wpbitswaitlist' && isset( $_GET['slug'] ) && $_GET['slug'] !='all' ) {
